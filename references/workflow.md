...
### Cinema 3.5 media is rejected before job creation

After one controlled attempt, confirm through provider history that no job was
created and close the attempt as `NOT_SUBMITTED`. Do not keep polling the same
IP gate. Read `web-ui-runtime.md`, open the dedicated browser, require the user
to complete login/2FA, bind the approved image explicitly as `Start Frame`, and
perform one visible paid preflight. Submit once, then bind the returned job with
`--submission-surface web_ui`; all waiting, cost truth, downloading, and QC
return to the normal CLI/state workflow.
