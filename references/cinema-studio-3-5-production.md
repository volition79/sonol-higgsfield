...
On CLI 1.1.19, Cinema 3.5 media can remain permanently rejected with
`IP check not finished for input media` even when the same provider-generated
image is selectable in the web UI. Treat one rejection plus provider-history
evidence of no job as `NOT_SUBMITTED`; do not start a timed retry loop. Read
[web-ui-runtime.md](web-ui-runtime.md), require the user's interactive web
login, preserve the approved shot settings, and submit once through the web UI.
Use CLI `generate list/get` afterward as the provider-truth and reconciliation
surface. This recovery was verified with a web job whose image was explicitly
stored as `medias[].role=start_image`.
