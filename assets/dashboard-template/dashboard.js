...
function render(){
  ...
  const ref=s.costs?.reference_estimates||{}; document.querySelector("#cost-panel").innerHTML=`<div class="cost-row"><span>REFERENCE ONLY</span><b>${ref.total_estimated_credits==null?esc(ref.status||"NOT COMPUTED"):esc(ref.total_estimated_credits)}</b></div><div class="cost-row"><span>ESTIMATE COVERAGE</span><b>${esc(ref.covered_shots||0)}/${esc(ref.total_shots||0)}</b></div><div class="cost-row"><span>ACTUAL</span><b>${esc(s.costs?.actual?.credits||0)}</b></div><div class="cost-row"><span>RECONCILIATION</span><b>${s.costs?.actual?.reconciliation_required?"REQUIRED":"CLEAR"}</b></div><div class="cost-row"><span>APPROVED CEILING</span><b>${esc(s.project?.cost_approval?.max_credits)}</b></div>`;
