...
const badge = value => { const v=String(value||"UNKNOWN"); const cls=/PASSED|APPROVED|LOCKED|COMPLETE|GENERATED/.test(v)?"good":/FAILED|REVISION|HOLD|CONFLICT/.test(v)?"bad":"warn"; return `<span class="state ${cls}">${esc(v)}</span>`; };
...
function approvalItems(s){
  ...
  if(s.project?.cost_approval?.status!=="APPROVED"){
    for(const [name,row] of Object.entries(s.costs?.scenarios||{})) if(row.estimated_credits!=null) items.push(`<div class="approval"><div><strong>${esc(name)}</strong><small>${esc(row.estimated_credits)} credits ceiling</small></div><button class="money" onclick='action({action:"approve_cost",scenario:${JSON.stringify(name)},max_credits:${Number(row.estimated_credits)}})'>APPROVE</button></div>`);
  }
...
function render(){
  ...
  const scenarios=s.costs?.scenarios||{}; document.querySelector("#cost-panel").innerHTML=Object.entries(scenarios).map(([k,v])=>`<div class="cost-row"><span>${esc(k)}</span><b>${v.estimated_credits==null?"NOT QUOTED":esc(v.estimated_credits)}</b></div>`).join("")+`<div class="cost-row"><span>ACTUAL</span><b>${esc(s.costs?.actual?.credits||0)}</b></div><div class="cost-row"><span>APPROVED CEILING</span><b>${esc(s.project?.cost_approval?.max_credits)}</b></div>`;
