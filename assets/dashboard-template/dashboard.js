...
function render(){
  ...
  document.querySelector("#grammar-grid").innerHTML=(s.shots||[]).map(x=>{const g=x.shot_grammar||{},d=x.director_intelligence||{};const core=[g.shot_size,g.angle,g.lens_family,g.movement].filter(Boolean).join(" · ");const perf=d.performance_direction?.selected_cues?.slice(0,3).join(" + ")||"미적용";const cam=d.camera_intent?.selected_strategy||"선택 전";const cx=d.complexity?`${d.complexity.level} ${d.complexity.score}`:"미검사";const lint=d.prompt_lint?.status||g.provider_binding?.prompt_lint?.status||"미검사";const retry=d.failure_analysis?.next_action||"판정 전";return `<div class="grammar-card"><div><span class="id">${esc(x.id)}</span>${badge(g.status)}</div><h3>${esc(g.dramatic_beat||"촬영 의도 미정")}</h3><p>${esc(core||"기법 미선택")}</p><small>${esc(g.why||"추천 → 선택 → 공급자 컴파일이 필요합니다.")}</small><small>연기 ${esc(perf)} · 카메라 ${esc(cam)} · 복잡도 ${esc(cx)} · 프롬프트 ${esc(lint)} · 재시도 ${esc(retry)}</small><footer>${esc(g.provider_binding?.provider||"NO PROVIDER")} / ${esc(g.provider_binding?.support_level||"UNVERIFIED")}</footer></div>`}).join("")||'<div class="empty">등록된 샷 문법이 없습니다.</div>';
