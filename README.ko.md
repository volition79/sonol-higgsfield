...
컷에서 새 시작 이미지를 즉시 합성합니다. 기본은 시작 이미지만 쓰는
...
좋은 프레임을 고를 수 있습니다. 스키마 v8은 시작 이미지 사전 검수, 첫
프레임의 화면비·콜라주/라벨·주체 가독성·첫 행동 적합성·화면 밖 공간 노출
위험을 검사합니다. 선택적 SSIM·PSNR 비교는 기술 증거이지 자동 합격 판정이
아닙니다. 첫 프레임 QC, 경계 분석, 잠긴 스토리 고정점과 이전 영상의 사용자
승인이 다음 생성을 통제합니다. 기본 마침 상태는 프롬프트로 지시하며, 마침
