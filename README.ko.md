...
- Higgsfield MCP는 선택 사항. 이 스킬은 라이브 CLI 스키마를 검색·공급자
  사실 확인 계약으로 사용하므로 MCP 없이도 작동

### 브라우저 실행과 로그인

Cinema Studio의 웹 전용 제어가 필요하거나 CLI가 잡 생성 전에 실패한 것이
확인되면 보이는 Higgsfield 브라우저 세션을 사용할 수 있습니다. 로그인,
2단계 인증, 캡차, 패스키, 동의는 사용자가 그 브라우저에서 직접 완료해야
합니다. 에이전트는 비밀번호, 인증 코드, 쿠키, 토큰을 요청하거나 검사하지
않습니다. 전용 프로필은 같은 컴퓨터의 세션을 보존하지만 다른 컴퓨터에서는
사용자가 다시 로그인해야 합니다.

호스트가 제공하는 macOS Computer Use 계열의 네이티브 브라우저 제어를 먼저
사용하고, 다음으로 Playwright/브라우저 MCP를 사용합니다. 내장된 로컬 CDP
폴백은 Windows Claude Code, Windows 브라우저를 제어하는 WSL, macOS, 데스크톱
Linux에서 작동하도록 구성했습니다.
...
폴백에는 Python 3.10 이상, Chrome/Edge/Chromium, Node.js 22 이상이 필요하며
`127.0.0.1`에만 연결하고 격리된 프로필을 사용합니다. Cinema Studio 3.5에서
이미지를 고르면 처음에는 `Reference`가 되므로, 유료 생성 한 번을 누르기 전에
반드시 `Start Frame`으로 바꾸고 화면에 `Start` 배지가 보이는지 확인합니다.
...
대표 숏에 크레딧을 쓰기 전, 현재 계약을 조회하고 실제 핵심 요구에 맞는
공급자를 보조 라우터로 판단할 수 있습니다.
...
라우터는 선택 이유만 설명하며 유료 잡을 제출하지 않습니다. Cinema 3.5를
선택하면 컴파일러가 검증된 `camera_style`, `light_scheme`, `color_grading`,
`genre`를 구조화 필드로 묶고, 정확한 이동·렌즈 지시는 프롬프트에 유지하며,
실행 모드 `model`과 보호된 `generate create` 경로를 사용합니다. 우열이
불확실하면 추측으로 전체 제작 모델을 바꾸지 않고 대표 숏 하나만 A/B합니다.
...
좋은 프레임을 고를 수 있습니다. 스키마 v11은 시작 이미지 사전 검수, 첫
...
스키마 v11은 각 공급자 시도가 CLI, 웹 UI, MCP, 기타 외부 표면 중 어디에서
제출됐는지도 기록합니다. 감독 보조 기능은 보이는 연기 단서, 카메라 대안 2개, 프롬프트 검사, 설명
...
알려진 CLI 응답인 잡 객체, 중첩 ID, UUID 하나만 담긴 리스트를 모두 식별하며,
`nsfw`, `moderated`, `blocked` 같은 검열 종료 상태는 유령 대기 상태로 남기지
않고 공급자 실패로 기록합니다.
웹에서 제출한 잡도 정확한 공급자 잡 ID로 정합하고 `web_ui` 제출 표면,
공급자, 내려받은 결과 경로, 실제 비용을 같은 원장에 보존합니다.
